...
async function _checkSupabaseConfig(){
  try{
    const { error } = await client.from('value_bets_feed').select('id').limit(1);
    if(error){
...
async function refreshSessionAndAccess(){
  // Login/VIP temporarily disabled — app runs fully public.
  session = null;
  accessActive = true;
  try{ setAuthButton(); }catch(e){}
...
async function loadBets(){
  ...
  try{
    // Public app: always read directly from value_bets_feed
    ...
    // Fallback if "id" doesn't exist / isn't sortable
    if(res.error){
      res = await client
        .from("value_bets_feed")
        .select("*")
    ...
    const normalized = rows.map(r => ({
      ...
      bet_end_date: r.bet_end_date ?? r.end_date ?? r.betEndDate ?? "",
      created_at: r.created_at ?? ""
    }));

    // Show only today's active bets (uses bet_date / bet_end_date / created_at)
    const todays = normalized.filter(isValueBetActiveToday);

    if(!todays.length){
      betsTable.innerHTML = '<div class="loading">No bets for today.</div>';
      renderSubscriptionGate();
      return;
    }

    // keep your existing UI
    betsTable.innerHTML = todays.map(b => `
      <div class="bet-row">
        <div class="bet-main">
