...
// Use a fixed timezone (UK) so "today" matches what you expect on the site,
// regardless of Supabase timestamps being stored in UTC.
function toTZYMD(d=new Date(), tz='Europe/London'){
  const dt = new Date(d);
...
// Backwards-compat helper (older code calls this)
function toLocalYMD(d=new Date()){
  return toTZYMD(d, 'Europe/London');
}
...
function isValueBetActiveToday(row){
  const today = toTZYMD(new Date(), 'Europe/London');

  // Your table should have bet_date (date). Some older versions used match_date.
  // If neither exists, fall back to created_at.
  const startRaw = row.bet_date ?? row.match_date ?? row.created_at;
  if(!startRaw) return false;

  const start = (typeof startRaw === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(startRaw))
    ? startRaw
    : toTZYMD(startRaw, 'Europe/London');

  const endRaw = row.bet_end_date ?? startRaw;
  const end = (typeof endRaw === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(endRaw))
    ? endRaw
    : toTZYMD(endRaw, 'Europe/London');
